...
class FirstMale implements ReportableInterface
{
    ...
    public function report(string $location, string $event, string $variance): ?ReportInterface
    {
        ...
        foreach ($summary->getResults()->iterate() as $result) {
            if ($result->getAthlete()->getGender() === 'Male') {
                return new Report(
                    $variance,
                    "{$result->getAthlete()->getName()} was the first male with a time of {$result->getTime()}."
                );
            }
        }
